...
if (isset($_GET['register'])) {
    ...
    try {
        ...
        if ($username === NULL || $password_hash === NULL) {
            $parse_error_headers = [];
            if (!$password_hash) {
                $parse_error_headers[] = registration_info_header('password parse error');
            }
            if (!$username) {
                $parse_error_headers[] = registration_info_header('username parse error');
            }
            load_page('login', 
                extra_headers: $parse_error_headers,
                alert_message: 'Felhasználónév és/vagy jelszó formátuma nem megfelelő!',
